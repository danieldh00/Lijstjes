...
test('een mutatie op een bekende lijst is toegestaan', () => {
  ...
  assert.equal(isAllowedMutation({ type: 'rename_list', entity_id: 'todo.boodschappen' }, valid), true);
  assert.equal(isAllowedMutation({ type: 'delete_list', entity_id: 'todo.boodschappen' }, valid), true);
});
...
test('een mutatie op een onbekende entity_id wordt geweigerd', () => {
  ...
  assert.equal(isAllowedMutation({ type: 'rename_list', entity_id: 'todo.iets_anders' }, valid), false);
  assert.equal(isAllowedMutation({ type: 'delete_list', entity_id: 'todo.iets_anders' }, valid), false);
});
